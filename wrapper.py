...
class Sequence(object):
    ...
    def score(self, x_test, y_test,fileToWrite,trainPath):
        ...
        if self.model:
            ...
            #mainAdjustPredTag(x_test_org=x_test_org,y_pred=y_pred,ARFileTrain=trainPath)
            #writeTupleArray(x_test_org,y_test,y_pred,fileToWrite)
            #mainAdjustPredTag(x_test_org=x_test_org,y_pred=y_pred)
            #showDiffTags(x_test_org,y_test,y_pred,"DiffTags.txt")
            #writeTupleArray(x_test_org, y_test, y_pred, fileToWrite="diff-tags.txt")
            #CRF_Checker(y_pred)


            for index in range(len(y_pred)):
                for eachIndex in range(len(y_pred[index])):
                    if("B-" in y_pred[index][eachIndex]):
                        y_pred[index][eachIndex]="I-"+y_pred[index][eachIndex][2:]

            merged_X = list(itertools.chain.from_iterable(y_pred))
            print(list(set(merged_X)))
